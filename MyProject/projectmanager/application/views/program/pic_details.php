	...
	<body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color')?>">
		...
		<script type='text/javascript'>
			var buTable = $('#buTable').DataTable({
				...
				ajax: 
				{
					url: "<?= base_url()?>program/ax_data_pic_detail/",
					type: 'POST',
					data: function ( d ) {
			         return $.extend({}, d, { 
			...
			$('#btnSave').on('click', function () {
				...
				{
					var url = '<?=base_url()?>program/ax_set_pic_detail';
					var data = {
						id_program_detail: <?= $id_program_detail ?>,
						id_pic: $('#id_pic').val(),
						id_user: $('#id_user').val(),
			...
			function ViewData(id_pic)
			{
				...
				{
					var url = '<?=base_url()?>program/ax_get_data_pic_detail_by_id';
					var data = {
						id_pic: id_pic
					};
			...
			function DeleteData(id_pic)
			{
				alertify.confirm(
					...
					function(){
						var url = '<?=base_url()?>program/ax_unset_pic_detail';
						var data = {
							id_pic: id_pic
						};
