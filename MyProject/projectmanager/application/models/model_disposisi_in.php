<?php
class Model_disposisi_in extends CI_Model
{
    public function getAllSurat($show = null, $start = null, $cari = null, $id_akun = null)
    {
        $this->db->select("a.id_surat, a.id_disposisi, b.nm_type_surat, c.nm_klasifikasi, d.nm_kategori, aaa.perihal, aaa.isi_surat, aaa.active, aaa.cdate ,e.nm_user, aaa.tgl_surat, a.cdate as tgl_disposisi");
        $this->db->from("tr_disposisi_int aa");
        $this->db->join("tr_disposisi a", "aa.id_disposisi = a.id_disposisi", "left");
        $this->db->join("tr_surat aaa", "aaa.id_surat = a.id_surat", "left");
        $this->db->join("ref_type_surat b", "aaa.id_type_surat = b.id_type_surat", "left");
        $this->db->join("ref_klasifikasi c", "aaa.id_klasifikasi = c.id_klasifikasi", "left");
        $this->db->join("ref_kategori d", "aaa.id_kategori = d.id_kategori", "left");
        $this->db->join("ref_user e", "a.cuser = e.id_user", "left");

        $session = $this->session->userdata('login');
        $this->db->where('aaa.id_perusahaan', $session['id_perusahaan']);
        $this->db->where('aa.id_akun', $id_akun);
        $this->db->where("(aaa.perihal  LIKE '%" . $cari . "%' ) ");
        $this->db->where("aaa.active", 3);
    ...
    public function get_count_surat($search = null, $id_akun)
    {
        ...
        $this->db->where('aa.id_akun', $id_akun);
        $this->db->where("aaa.active", 3);      
        $this->db->like("aaa.perihal ", $search);
        $count['recordsFiltered'] = $this->db->get()->row_array()['recordsFiltered'];

        $this->db->select(" COUNT(a.id_surat) as recordsTotal ");
        $this->db->from("tr_disposisi_int aa");
        $this->db->join("tr_disposisi a", "aa.id_disposisi = a.id_disposisi", "left");
        $this->db->join("tr_surat aaa", "aaa.id_surat = a.id_surat", "left");
        $this->db->where('aaa.id_perusahaan', $session['id_perusahaan']);
    ...
    public function getAlldisposisi($id_surat = null, $id_akun = null)
    {
    ...
    public function get_count_disposisi($id_surat = null, $id_akun = null)
    {
        $count = array();
        $session = $this->session->userdata('login');
    ...
    public function get_selected_dispo($id_disposisi)
    {
        $this->db->select("group_concat(DISTINCT d.id_alias) id_alias");
        $this->db->from("tr_disposisi a");
        $this->db->join("tr_disposisi_note b", "a.id_disposisi = b.id_disposisi");
        $this->db->join("ref_user c", "b.cuser = c.id_user");
        $this->db->join("tr_disposisi_alias d", "a.id_disposisi = d.id_disposisi");
        $this->db->where("a.id_disposisi", $id_disposisi);
        
        
        return $this->db->get();
    }
    public function get_chatdispo($id_disposisi)
    {
        $this->db->select("a.id_disposisi, d.id_alias, a.cuser, c.nm_user, a.nm_note, a.cdate");
        // $this->db->from("tr_disposisi a");
        // $this->db->join("tr_disposisi_note b", "a.id_disposisi = b.id_disposisi");
        $this->db->from("tr_disposisi_note a");
        $this->db->join("tr_disposisi b", "a.id_disposisi = b.id_disposisi");
        $this->db->join("ref_user c", "a.cuser = c.id_user");
        $this->db->join("tr_disposisi_alias d", "a.id_disposisi = d.id_disposisi");
        $this->db->where("a.id_disposisi", $id_disposisi);
        $this->db->group_by("a.nm_note");
        $this->db->order_by("a.cdate", "ASC");

        return $this->db->get();
    }
    public function get_count_chatdispo($cari = null, $id_surat)
    {
        $count = array();
        $session = $this->session->userdata('login');
