				...
				<div class="row">
					<div class="col-lg-5">
						<div class="panel panel-default">
							<div class="panel-heading">
								...
								<div class="col-md-6">
								<select class="form-control select2" style="width: 100%;" id="akses_akun_approval" name="akses_akun_approval">
									<!-- <option value="0">-- Akses Akun --</option> -->
									<?php
										foreach ($akses_akun as $ak) {
									?>
								...
								<div class="col-md-6">
								<select class="form-control select2" style="width: 100%;" id="filter_status_approval" name="filter_status_approval">
					...
					<div class="col-lg-7">
						<ul class="nav nav-tabs">
							...
							<button type="button" id="BtnApproveSurat"  class="pull-right btn btn-success pull-right d-none ">
								<i class='fa fa-check'></i> SETUJUI SURAT
							</button>
							<button type="button" id="BtnRejectSurat" style="margin-right: 10px;" class="pull-right btn btn-danger pull-right d-none">
									...
									<div class="panel panel-primary">
										...
										<div id="collapse6" class="panel-collapse collapse">
											<div class="panel-body">
												<div class="panel panel-default">
													<div class="panel-heading">
														<h4 class="pull-left">List Attachment</h4>
														<button class="btn btn-primary pull-right" onclick='addAttachment(0)'>
...
<script>
	...
	let id_akun_selected = $('#akses_akun_approval').val();

	$(document).ready(function() {
		$(function () {
			$("akses_akun_approval").change();
		});
	...
	const get_data_surat = (id_surat) => { // get data surat by id
		const url = '<?= base_url() ?>approval/ax_get_data_by_id';
		...
		return $.ajax({
			...
			$("#BtnApproveSurat").show();
			$("#BtnRejectSurat").show();
			$("#SuratBtnEdit").show();
			// pass data 
			console.log(data);
			if(data.active == 3){
				$("#BtnApproveSurat").hide();
				$("#BtnRejectSurat").hide();
				$("#SuratBtnEdit").hide();
			}
...
<script type='text/javascript'>
	var buTable ;
	const getSuratByApprovalAkun = (id_akun = null, status = null) => {
		var data = {
			id_akun: id_akun,
			status: status,
		};
		buTable = $('#buTable').DataTable({
			...
			"serverSide": true,
			ajax: {
				url: "<?= base_url() ?>approval/ax_data_surat/",
				type: 'POST',
				data: data
			},
			columns: [

				{
					...
					render: function(data, type, full, meta) {
						...
						if(full['status'] == 1 ){ // span color status surat
							str += `<span class="label label-warning">Waiting</span> | `;
						}else if(full['status'] == 2 ){
							str += `<span class="label label-primary">Proses Approval</span> | `;

						}else{
							str += `<span class="label label-success">Di Setujui</span> | `;
						}
						if(full['type_approval'] == 1 ){ // span color type approval
							str += `<span class="label label-warning">Paraf</span>`;
						}else if(full['type_approval'] == 2 ){
							str += `<span class="label label-success">Tanda Tangan</span>`;
						}

						str +=` - ${full['nm_type_surat']} - ${full['nm_klasifikasi']} - ${full['nm_kategori']}- ${full['perihal']}
						</span>
						`;
						return str;
					}
	...
	getSuratByApprovalAkun(); // first call surat approval

	// isi surat berdasarkan akses akun approval di select
	$('#akses_akun_approval').on('change', function() {
		// alert( this.value );
		let id_akun = this.value;
		getSuratByApprovalAkun(id_akun,null); // reload table berdasarkan id_akun
		id_akun_selected = id_akun;
		$("#BtnApproveSurat").hide();
		$("#BtnRejectSurat").hide();
		$("#filter_status_approval").change();

	});
	// isi surat berdasarkan akses akun approval di select
	$('#filter_status_approval').on('change', function() {
		// alert( this.value );
		console.log(`id akun ${id_akun_selected}, status approval : ${this.value}`)
		getSuratByApprovalAkun(id_akun_selected,this.value); // reload table berdasarkan id_akun
		$("#BtnApproveSurat").hide();
		$("#BtnRejectSurat").hide();
	});
	...
	$("#BtnApproveSurat").on("click", function () {
	if (id_surat_selected == null) {
		alertify.error(`Pilih Surat Terlebih Dahulu !`);
		return;
	}else if(id_akun_selected == null){
		alertify.error(`Pilih Akun Akses Terlebih Dahulu !`);
		return;
	}

	console.log(id_surat_selected);
	console.log(id_akun_selected);
	// return;
	...
	alertify
		...
		`
					...
					<b style="color:red;">Note* : Pastikan Surat Sudah Benar</b>.
					`,
		function (ok) {
			if (ok) {
			var url = `${base_url}approval/ax_kirim_surat_approval`;
			var data = {
				id_surat: id_surat_selected,
				id_akun: id_akun_selected,
			...
			$.ajax({
				...
				var data = JSON.parse(data);
				if (data["status"] == "success") {
					alertify.success("Surat Berhasil Di Approve");
					buTable.ajax.reload();
					$('.nav-tabs a[href="#preview_pdf"]').tab("show"); // pindah tab preview pdf after edit
					$('#showpdf').hide();
	...
	$("#BtnRejectSurat").on("click", function () {
	if (id_surat_selected == null) {
		alertify.error(`Pilih Surat Terlebih Dahulu !`);
		return;
	}
	...
	alertify
		.confirm(
		`
					Apakah anda yakin untuk reject surat ini ? <br><br>
					<b style="color:red;">Note* : Data surat akan di kembalikan ke awal / draft !</b>.
					`,
		function (ok) {
			if (ok) {
			var url = `${base_url}approval/ax_reject_surat`;
			var data = {
				id_surat: id_surat_selected,
			};
			...
			$.ajax({
				...
				var data = JSON.parse(data);
				if (data["status"] == "success") {
				alertify.success('Data Surat Berhasil Di Reject.');
				buTable.ajax.reload();
				$('.nav-tabs a[href="#preview_pdf"]').tab("show"); // pindah tab preview pdf after edit
				$('#showpdf').hide();
				$("#BtnRejectSurat").hide();
    ...
    // $("#showpdf").html(`<iframe style="width: 100%; height: 650px;" id="pdf-preview" src="${url}"></iframe>`);

		return new Promise((resolve, reject) => {
		setTimeout(function() {
			$("#showpdf").html(`<object id="pdf-preview" style="width: 100%; height: 650px;" data="${url}">`);
			$('#showpdf').show();
...
const getInternalSelected = () => {
    ...
    buTableInter = $('#buTableInter').DataTable({
        ...
        ajax: 
        {
            url: `${base_url}approval/ax_data_surat_internal/`,
            type: 'POST',
            data: data
        },
        columns: 
    ...
    $('#btnSaveInter').on('click', function () {
        ...
        {
            var url = `${base_url}approval/ax_set_data_internal`;
            var data = {
    ...
    function DeleteDataInter(id_surat_alias)
    {
        alertify.confirm(
            ...
            'Are you sure you want to delete this data?', 
            function(){
                var url = `${base_url}approval/ax_unset_data_internal`;
                var data = {
...
//============================ JS COLLAPSE APPROVAL ============================
	...
	if (id_surat_selected == "" || id_surat_selected == 0) {
		...
		var url = `${base_url}approval/ax_set_data_approval`;
		var data = {
	...
	if (id_approval == 0) {
		...
		var url = `${base_url}approval/ax_get_data_approval_by_id`;
		var data = {
	...
	alertify.confirm(
		...
		"Are you sure you want to delete this data?",
		function () {
		var url = `${base_url}approval/ax_unset_data_approval`;
		var data = {
			id_approval: id_approval,
		};
...
const getExternalSelected = () => {
    ...
    buTableEx = $('#buTableEx').DataTable({
        ...
        ajax: 
        {
            url: `${base_url}approval/ax_data_surat_external/`,
            type: 'POST',
            data: data
    ...
    $('#btnSaveEx').on('click', function () {
        ...
        {
            var url = `${base_url}approval/ax_set_data_external`;
            var data = {
    ...
    function ViewDataEx(id_surat_external)
    {
        ...
        {
            var url = `${base_url}approval/ax_get_data_external_by_id`;
            var data = {
    ...
    function DeleteDataEx(id_surat_external)
    {
        alertify.confirm(
            ...
            'Are you sure you want to delete this data?', 
            function(){
                var url = `${base_url}approval/ax_unset_data_external`;
                var data = {
                    id_surat_external: id_surat_external
                };
...
$('#upload_attach').submit(function(e) {
    ...
    if ($('#id_surat_attach').val() == '') {
        ...
        $.ajax({
            url: `${base_url}approval/ax_surat_upload_attachment/`,
            type: "post",
...
const getAttachSelected = () => {
    ...
    buTableAttach = $('#buTableAttach').DataTable({
        ...
        ajax: 
        {
            url: `${base_url}approval/ax_data_attachment/`,
            type: 'POST',
            data: data
